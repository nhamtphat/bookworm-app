...
import {DecreaseQuantity, IncreaseQuantity, EmptyCart} from "../_store/actions";
...
function Cart(props) {
    ...
    return (
        <Layout>
            ...
            <section className={thankYou ? "d-none" : "section-content bg padding-y pt-0"}>
                                    ...
                                    {props.cart.map((item, key) => (
                                        <tr className="border-top" key={key}>
                                            <td>
                                                <Link to={"/books/" + item.product.id} target='_blank'>
                                                    <figure className="itemside align-items-center">
                                                        <div className="aside">
                                                            <img src={item.product.book_cover_photo}
                                                                 className="img-sm"/>
                                                        </div>
                                                        <figcaption className="info">
                                                            <span
                                                                className="title text-dark">{item.product.book_title}</span>
                                            ...
                                            <td>
                                                <div className="price-wrap">
                                                    <var
                                                        className="price">${TotalPrice(item.product.final_price, item.quantity)}</var>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
