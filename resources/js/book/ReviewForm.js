...
export default function ReviewForm({book, fetchData}) {
    ...
    function handleSubmit(event) {
        ...
        axios.post(`/api/books/${book.id}/reviews`, params)
            .then(response => {
                fetchData()
                resetForm()
                ...
                <form onSubmit={e => handleSubmit(e)}>
                    <div className="form-row">
                        <div className="col form-group">
                            ...
                            <input type="text" className="form-control"  maxLength="120" value={title} required={true} onChange={e => handleTitleChange(e)} />
                        </div>
                    </div>
                    <div className="form-group">
                        <label>Details please! Your review helps other shoppers.</label>
                        <textarea className="form-control" rows="3" value={details} onChange={e => handleDetailsChange(e)} ></textarea>
                    </div>
                    <div className="form-group">
                        <label>Select a rating star?</label>
                        <select className="form-control" value={ratingStart} onChange={e => handleRatingStarChange(e)} required={true} >
                            {[1,2,3,4,5].map(star => (
